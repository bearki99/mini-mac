...
const Row: React.FC<RowProps> = ({ id, onkeydown }) => {
  ...
  return (
    <div className="flex flex-col w-full h-12">
      <div>
        <span className="mr-2 text-yellow-400">bear</span>
        <span className="mr-2 text-green-400">@macbook-pro</span>
        <span className="mr-2 text-blue-400">~</span>
        <span
          id={`terminal-currentDirectory-${id}`}
...
const Help = () => {
  return (
    <ul key={Math.random().toString()} className="list-disc ml-6 pb-1.5">
      ...
      <li>
        <span className="text-purple-400">Open/Close {"<AppId>"}</span> -
  ...
  return (
    <div className="flex w-full h-6">
      <span className="mr-2 text-red-400">
        command not found: <span className="text-purple-400">{command}</span>
      </span>
    </div>
  );
};

export { Row, Help, CommandNotFound };
